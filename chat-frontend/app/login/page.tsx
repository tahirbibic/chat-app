...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: C.prussian, color: C.text }}>
      <form onSubmit={handleLogin} className="w-full max-w-sm p-6 sm:p-8 rounded-2xl shadow-2xl" style={{ backgroundColor: C.indigo, border: `1px solid ${C.border}` }}>
        <h1 className="text-2xl sm:text-3xl font-bold mb-2 text-center">Welcome back</h1>
        <p className="text-center text-sm mb-6" style={{ color: C.muted }}>Log in to continue</p>
        ...
        <p className="text-center text-sm mt-4" style={{ color: C.muted }}>
          No account?{" "}
          <button type="button" onClick={() => router.push("/register")} style={{ color: C.dusk }} className="hover:underline">
            Register
          </button>
        </p>
      </form>
